...
# Apparently in Unix, preference is given to arguments vs standard input, so first check whether no arguments have been provided
# If notes are piped to the script, they are present in standard input
# So test if standard input is empty or not, if it isn't, assume user piped notes into it and use makepdf to make the pdf
# How to direct the output? Standard is inside $NOTES_DIR. User can change by adding -o flag with filename.
isNotEmptyRegex="[[:alnum:]]"
echo "$allNotes"

if [ $# -le 1 ] && [[ "$allNotes" =~ $isNotEmptyRegex ]] # [ -p /dev/stdin ]
then
	# data was piped and maximally 1 argument given to script (should be -x)
	if [ $# -eq 1 ] && [ "$1" != "-x" ]
	then
		exit 1
	else
	...
	# take all .md/.mkd/.markdown files, and makepdf
	pipedNotes=$(while IFS='$\n' read -r line
	do
		case "$line" in
			(*\.md)
				;&
	...
	makepdf "master" "$pipedNotes" "$customizationDirectory"
	# exit script
	exit 0
fi
